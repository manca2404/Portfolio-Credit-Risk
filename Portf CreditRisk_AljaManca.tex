...
movements. Losses can occur even without defaults, because a worsening credit quality
makes credit instruments worth less in the market. CreditMetrics is the most 
...
Probability of default (PD) is the likelihood that over a specified period a borrower will
...
Reduced-form, or intensity-based, models treat default as an 
...
\text{PD} = f(DD),
\]
calibrated from historical default data. In practice, the KMV methodology does not interpret this probability directly as a theoretical PD derived from the Merton model, but rather as an Expected Default Frequency (EDF), which is obtained through empirical calibration based on large historical datasets of observed defaults. This PD can then be used as an input into 
...
\]
It should be noted that the normality assumption applies to the latent variables rather than to the default indicators themselves, which follow a Bernoulli distribution. The copula framework allows flexible modelling of dependence while preserving arbitrary marginal default probabilities.
...
Under the Vasicek model, the asymptotic portfolio default rate admits a closed-form solution, which allows analytical computation of loss quantiles and forms the theoretical foundation of the Basel II and Basel III Internal Ratings-Based (IRB) capital requirements.
